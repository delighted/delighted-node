...
var request = function(serverConfig, requestConfig){
	...
	serverConfig.port = (requestConfig.secure ? 443 : 80);

	http.globalAgent.maxSockets = 1024;

	var enableLogs = false || !!requestConfig.enableLogs;
	...
	var makeRequest = function(method, url, data, headers, otherOptions){
		...
		if(defaultSendDataType == 'json'){
			...
			// put data into a form format "field=value&field2=value2"
			var formData = [];
		...
		if(requestConfig.isDev && enableLogs){
			console.log('------ OPTIONS ------');
			console.log(options);
			console.log('---------------------');
			console.log('------ HEADERS ------');
